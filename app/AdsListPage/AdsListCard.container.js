...
class AdsListCard extends React.Component{
  ...
  componentWillMount() {
    console.log('AdsListCard', this.props, this.props.allAdses);
  ...
  render() {
    ...
    const transportType = ads.transportType;
    ...
    if(transportType){
      const name = listTransportTypes[transportType-1];
      subtitle += ` by ${name}`;
    }
    ...
    const detailIsURL = ads.detailUrl.startsWith('https://') || ads.detailUrl.startsWith('http://');
    ...
    return (
      ...
      <Card id={ads.id}>
        ...
        <CardActions>
          {!hideDetail && ads.detailUrl && detailIsURL ?
            ...
            {!hideDetail && ads.id ? <A href={`/trip/${ads.id}`}>
              <Button>
                Questions & Take Me Too
              </Button>
            </A> : null}
          ...
          {!detailIsURL && onTakeMeToo ? 
            <Button onClick={onTakeMeToo} >
              Take Me Too!
            </Button> : null
          }
